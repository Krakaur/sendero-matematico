async (page) => {
  ...
  await page.reload();
  await page.getByText('Exploración del río', {exact:true}).waitFor();
  const verify = async () => page.evaluate(async () => {
    const before = JSON.parse(sessionStorage.getItem('upgrade-before'));
    const after = JSON.parse(localStorage.getItem('sendero.profile.'+sessionStorage.getItem('sendero.active')));
